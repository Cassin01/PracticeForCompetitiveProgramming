...
impl<T: Clone> Combo<T> {
    fn new(chunk_len: i32, data: Vec<T>) -> Combo<T> {
        let d_len = data.len();
        let min = 2usize.pow(chunk_len as u32) - 1;
        let max = 2usize.pow(d_len as u32)
                - 2usize.pow((d_len - chunk_len as usize) as u32);
        Combo {
            data_len: d_len,
    ...
    fn get_chunk(&self) -> Vec<T> {
        let b = format!("{:1$b}", self.mask, self.data_len);
        b
            .chars()
            .enumerate()
            .filter(|&(_,e)|e=='1')
