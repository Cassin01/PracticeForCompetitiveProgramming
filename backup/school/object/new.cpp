...
void Cal_Total(struct Price, struct Order*);
void Disp_Total(struct Order);
void Set_Hamburger(struct Order*, int);
void Set_Potato(struct Order*, int);
void Set_Drink(struct Order*, int);
...
void Cal_Total(struct Price p, struct Order *o) {
  o -> total = p.hamburger * o -> hamburger
             + p.potato   *    o -> potato
             + p.drink    *    o -> drink;
}
